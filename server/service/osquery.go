...
// deployment — including queries of other teams, queries outside its label
// scope, and queries that are not scheduled at all — and have them persisted in
// admin-visible query reports.
//
// This runs on every result log submission, so it must stay cheap. The query
// rows in queriesDBData come from Datastore.QueryByName and the host's label
// membership from Datastore.HostLabelIDs, both of which are cached in
// cached_mysql, and the label membership is only looked up when at least one of
// the reported queries is label-scoped.
//
...
// case where this is called). Platform and min osquery version are deliberately
// not enforced here: those are applied by osquery itself, not by Fleet when it
// hands out the schedule.
func (svc *Service) queriesScheduledForHost(
	...
	var labelScopedNames, notScheduledNames []string
	...
	for reportedName, dbQuery := range queriesDBData {
		...
		case len(dbQuery.LabelsIncludeAny) > 0 || len(dbQuery.LabelsIncludeAll) > 0:
			// Needs the host's label membership, checked below.
			labelScopedNames = append(labelScopedNames, reportedName)
			continue
		default:
	...
	if len(labelScopedNames) > 0 {
		hostLabelIDs, err := svc.ds.HostLabelIDs(ctx, host.ID)
		if err != nil {
			// Fail closed: without the host's label membership we cannot tell whether
			// these queries are part of its schedule.
			svc.logger.ErrorContext(ctx, "get host label IDs", "err", err, "host_id", host.ID)
			notScheduledNames = append(notScheduledNames, labelScopedNames...)
		} else {
			hostLabels := make(map[uint]struct{}, len(hostLabelIDs))
			for _, labelID := range hostLabelIDs {
				hostLabels[labelID] = struct{}{}
			}
			for _, reportedName := range labelScopedNames {
				if hostInQueryLabelScope(queriesDBData[reportedName], hostLabels) {
					scheduled[reportedName] = struct{}{}
				} else {
					notScheduledNames = append(notScheduledNames, reportedName)
				}
...
// hostInQueryLabelScope reports whether a host with the given label membership
// is in the label scope of the query. It mirrors the label filtering done by
// Datastore.ListScheduledQueriesForAgents.
func hostInQueryLabelScope(query *fleet.Query, hostLabelIDs map[uint]struct{}) bool {
	// include_all: the host must be a member of every listed label.
	for _, label := range query.LabelsIncludeAll {
		if _, ok := hostLabelIDs[label.LabelID]; !ok {
			return false
		}
	}

	// include_any: the host must be a member of at least one of the listed labels.
